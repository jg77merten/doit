...
class FinalView_View_Helper_FormAllErrors extends Zend_View_Helper_FormElement
{
     ...
     * @return string
     */
    public function formAllErrors(Zend_Form $form, array $options = null)
    {
        ...
        if ($form->isErrors() || count($form->getErrorMessages())) {
            $errors = array();
            
            foreach ($form->getElements() as $element) {
                if ($element->hasErrors()) {
                    $errors[] = $element->getLabel() . ': ' . 
                        $this->view->FormErrors($element->getMessages(), array('escape' => false));
                }
            }
            
            $html  = $start;
            if (count($form->getErrorMessages())) {
                $html .= $this->view->FormErrors($form->getErrorMessages(), 
                    array('class' => 'common_errors', 'escape' => false));
            }
            $html .= 
